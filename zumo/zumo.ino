...
#define SPEED           75  // Maximum motor speed when going straight; variable speed when turning
#define TURN_BASE_SPEED 150 // Base speed when turning (added to variable speed)
...
void setup(){
	...
	motors.setLeftSpeed(TURN_BASE_SPEED);
	motors.setRightSpeed(-TURN_BASE_SPEED);
...
void loop(){
	...
	heading = averageHeading(); // Heading is given in degrees away from the magnetic vector, increasing clockwise
	relative_heading = relativeHeading(heading, target_heading); // This gives us the relative heading with respect to the target angle
	...
	// 	if(speed < 0) speed -= TURN_BASE_SPEED;
	// 	else speed += TURN_BASE_SPEED;
